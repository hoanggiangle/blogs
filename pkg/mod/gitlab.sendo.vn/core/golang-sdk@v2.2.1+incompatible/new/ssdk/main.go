...
const pkgName = "gitlab.sendo.vn/core/golang-sdk/new"
const pkgNativePath string = "gitlab.sendo.vn" + PATHSEP + "core" + PATHSEP + "golang-sdk" + PATHSEP + "new"
...
func findTemplateDir(tmpl string) string {
	p, _ := filepath.Abs(fmt.Sprintf("%s/ssdk/%s-skeleton", findSdkPath(), tmpl))
	return p
}
